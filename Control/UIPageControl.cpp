...
#define PAGE_OPT_GROUP _T("PAGE_OPT_GROUP")
	...
	void CPageControlUI::SetMaxPages(int nPages, int nShowNum /*= 6*/)
	{
	...
	void CPageControlUI::SetMaxChar(int nNum /*= 2*/)
	{
		m_EdtPageNo->SetMaxChar(nNum);
	}

	void CPageControlUI::SetFont(int nFont)
	{
		m_nFont = nFont;
	...
	void CPageControlUI::SetPageSelectedTextColor(DWORD cr /*= 0xff00ff00*/)
	{
		m_dwSelTextColor = cr;
		for (int i = 0; i < m_OptPageNoArr.GetSize(); i++)
		{
	...
	void CPageControlUI::SetPageSelectedBkColor(DWORD cr)
	{
	...
	void CPageControlUI::SetPageNormalTextColor(DWORD cr /*= 0xff000000*/)
	{
	...
	void CPageControlUI::SetPageBkColor(DWORD cr /*= 0x00000000*/)
	{
	...
	void CPageControlUI::SetPageHotBkColor(DWORD cr)
	{
		m_dwHotBkColor = cr;
		m_BtnPrevious->SetHotBkColor(m_dwHotBkColor);
		m_BtnNext->SetHotBkColor(m_dwHotBkColor);
	...
	void CPageControlUI::SetPageHotTextColor(DWORD cr)
	{
		m_dwHotTextColor = cr;
	...
	void CPageControlUI::SetPageNoSize(int w /*= 30*/, int h /*= 30*/)
	{
		m_szPage.cx = w;
		m_szPage.cy = h;
		UpdatePosition();
	...
	void CPageControlUI::SetGotoEditSize(int w /*= 50*/, int h /*= 30*/)
	{
		m_szGotoEdit.cx = w;
		m_szGotoEdit.cy = h;
		UpdatePosition();
	...
	void CPageControlUI::SetGotoEditBorderColor(DWORD cr /*= 0x00000000*/)
	{
		m_EdtPageNo->SetBorderColor(cr);
	}

	void CPageControlUI::SetGotoEditBorderSize(int size /*= 1*/)
	{
		m_EdtPageNo->SetBorderSize(size);
	}

	void CPageControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
	...
	void CPageControlUI::ResetAllPages()
	{
	...
	void CPageControlUI::UpdatePosition()
	{
		int controlWidth = 0;
	...
	void CPageControlUI::UpdatePageNo(int iDelta, bool bReset)
	{
		if (bReset)
		{
			for (int i = 0; i < m_OptPageNoArr.GetSize(); i++)
	...
	void CPageControlUI::UpdateItemIndex()
	{
		int nIndex = 1;
		for (int i = 0; i < m_OptPageNoArr.GetSize(); i++)
		{
	...
	void CPageControlUI::GotoPage(int nPageNo, bool bAutoGotoOneWhenError)
	{
	...
	int CPageControlUI::GetIndexByPageNo(int nPageNo)
	{
	...
	void CPageControlUI::SelectPage(int nPageIdx)
	{
	...
	int CPageControlUI::GetCurSel()
	{
		return m_nSelPageNo;
	}

	void CPageControlUI::UpdateMoreBtnVisiable()
	{
	...
	bool CPageControlUI::IsExistNextMore()
	{
		bool bVisiable = m_BtnNextMore->IsVisible();
		return bVisiable;
	}

	bool CPageControlUI::OnOptionSelChanged(void* p)
	{
	...
	bool CPageControlUI::OnBtnClick(void* p)
	{
		TNotifyUI* notify = (TNotifyUI*)p;
	...
	int CPageControlUI::GetPageNoByControl(CControlUI* pcon)
	{
		if (!pcon)
		{
			return -1;
